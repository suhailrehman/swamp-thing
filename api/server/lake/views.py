...
class MetaViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        try:
            last_crawl = CrawlJob.objects.latest(
                field_name='start_time').start_time.strftime('%Y-%m-%d %H:%M')
        except Exception:
            last_crawl = 'None'
...
class TopicViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request):
        queryset = self.get_queryset()
        return Response([{'topic': str(q.topic_word), 'count': q.crawled_item.all().count()} for q in queryset])
...
class KeywordViewSet(viewsets.ModelViewSet):
    ...
    def cloud(self, request):
        ...
        return Response({'cloud': [[q.keyword, q.scores.count()] for q in keywords]})
...
class SklumaViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        try:
            ...
            for colname, colproperties in columns.iteritems():
                ...
                if(serialized_column.is_valid()):
                    ...
                    print "error", serialized_column.errors
                    errors.append(
                        {'column': colname, 'column_errors': serialized_column.errors})
        ...
        try:
            ...
            for topic in topics:
                try:
                    ...
                    # TODO: Failure is an option, don't store column.
                    errors.append(
                        {'topic': topic, 'topic_errors': serialized_topic.errors})
            ...
            for keyword, score in keywords.iteritems():
                ...
                try:
                    kwscore = KeywordScore(
                        keyword=existing_keyword, score=score, crawled_item=crawled_item)
                    kwscore.save()
                    num_keywords += 1
        ...
        added = {'number of colums': num_cols,
                 'number of topics': num_topics, 'number of keywords': num_keywords}
